...
这是 CodexQuotaTray 0.5.0 的 WinUI 3 正式交付候选。默认模式连接本机 Codex App Server 并读取真实额度；`--demo` 只用于开发演示。

0.5.0 使用 message-only HWND 承载托盘回调，另一个隐藏 tool window 接收 Explorer 重建和系统恢复广播。窗口外轮廓只由 DWM 圆角裁剪；窗口置顶并可从标题区自由拖动，本次运行内保留最后位置。标题、套餐、状态和刷新按钮采用紧凑单行布局，额度重置时间合并显示；额度卡片与系统轮廓采用协调圆角，进度条为 12 DIP 原生胶囊。窗口高度直接取底部可见操作行的实际边界并追加设计内边距，不再保留 ScrollViewer 的旧视口高度。自动刷新与手动刷新共用同一按钮忙碌状态。
