...
0.3.2 uses a per-user, no-admin Inno Setup installer containing the folder-based, self-contained x64 WinUI application. The older Rust ZIP workflow remains only as a regression and rollback baseline.
...
- WinUI 0.3.2 发布验证使用 .NET host 的 `RT_GROUP_ICON #32512`，并确认 16/20/24/32/40/48/64/128/256 帧随 EXE 和安装器一起交付。
