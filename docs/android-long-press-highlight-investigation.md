...
状态：方案 A 已否决，方案 B 已按用户验收确认符合当前交互目标并作为当前实现；本文件记录实现边界与后续可调参数，不改变现行产品合同。
...
本轮先实现方案 A 后进行用户验收，结论是方案 A 不可接受；方案 B 随后实现并已确认符合当前交互目标：

1. **方案 A（已否决）**：未选中 Tab 越过现有拖动门槛、成功接管后才接入高光。
   用户验收确认：长按不松手时，必须先任意滑动一段距离才有高光；一旦停止滑动就没有高光。
   这不满足“静止长按也应保持高光”的目标，故不再作为实现方案。
2. **方案 B（已确认符合）**：只要当前按压拥有未选中 Tab 的 preview，就让高光随胶囊实际位置
   接近目标连续增加，与已有形变同步；不等待系统长按、不新增离散 reveal。它接受较慢 tap
   可能出现少量高光的取舍。

方案 B 已按用户验收确认符合当前交互目标；若后续设备复核发现上述取舍不可接受，再重新评估
静止长按的实现边界。当前没有证据支持一个同时满足全部限制的第四条路线。
...
## 方案 B：静止长按随预览贴合连续发光（已确认符合）

本轮接受“较慢的 tap 可能有少量高光”的取舍并实现这一路线，用户验收已确认其符合当前交互
目标。亮度是已有预览动作的一部分，而不是预览结束后另起一次提示。

实现只使用当前有效按压、`previewIndex`、胶囊实际位置和已有形变进度：
...
上式表达“越接近越亮”。当前实现取 `nearDistance = 0.08`、`farDistance = 0.45`，高光强度为
`clamp(pressProgress, 0, 1) * contact`，只在同一有效 press 拥有 preview 时生效。
这两个距离是 Debug 可调参数，后续可根据真机对比调整；未引入额外毫秒阈值。
...
- 较慢的 tap 只要持续到贴合区域就会亮；这是方案 B 已确认接受的交互取舍。
...
本轮用户验收已确认方案 B 符合；以下场景保留为后续回归验收标准：
