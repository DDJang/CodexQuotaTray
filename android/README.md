...
始终立即执行。Token 使用量有独立的周期 WorkManager：开启后台自动同步且仍有 Windows 配对时才调度；
Worker 只在 Wi-Fi 上执行，移动网络会直接跳过。该任务不影响额度后台刷新。
...
- 同步页分别控制剩余额度的打开页面自动刷新/后台自动刷新，及 Token 使用量的打开统计页自动同步/后台自动同步；
  两者频率均可选 15 分钟、30 分钟或 1 小时，电池优化入口独立于两个开关。
