...
internal object AndroidLanDiagnosticsFormatter {
    ...
    fun format(
        ...
        return buildString {
            ...
            appendLine()
            appendLine("Last recorded attempt:")
            ...
            appendLine()
            appendLine("Last success:")
            appendLine("timestamp=${success?.let(::formatUtc) ?: "unavailable"}")
            appendLine("attempt=unavailable (not persisted separately)")
            appendLine()
            appendLine("Last failure:")
            appendLine("timestamp=${pairing?.lastLanFailureAtMillis?.let(::formatUtc) ?: "unavailable"}")
            appendLine("phase=${pairing?.lastLanFailurePhase ?: "unavailable"}")
            appendLine("attempt=unavailable (not persisted separately)")
            appendLine()
            appendLine("Network:")
