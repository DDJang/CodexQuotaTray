// Adapted from Kyant0/AndroidLiquidGlass at the pinned commit below.
...
suspend fun PointerInputScope.upstreamInspectDragGestures(
    ...
    awaitEachGesture {
        ...
        val upEvent = upstreamDrag(drag.id) { onDrag(it, it.positionChange()) }
        if (upEvent == null) onDragCancel() else onDragEnd(upEvent)
    }
}

private suspend inline fun AwaitPointerEventScope.upstreamDrag(
    pointerId: PointerId,
    ...
    while (true) {
        val change = upstreamAwaitDragOrUp(pointer) ?: return null
...
private suspend inline fun AwaitPointerEventScope.upstreamAwaitDragOrUp(
    pointerId: PointerId,
): PointerInputChange? {
    var pointer = pointerId
    while (true) {
